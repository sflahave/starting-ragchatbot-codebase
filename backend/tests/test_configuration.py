...
class TestConfiguration:
    ...
    def test_critical_max_results_bug(self):
        ...
        assert config.MAX_RESULTS > 0, "MAX_RESULTS must be positive for searches to return results"
    ...
    def test_anthropic_model_format(self):
        ...
        assert "claude" in model.lower(), f"Model '{model}' doesn't appear to be a Claude model"
    ...
    def test_embedding_model_format(self):
        ...
        valid_prefixes = ['all-MiniLM', 'all-mpnet', 'sentence-transformers', 'distilbert']
        is_valid_format = any(prefix in embedding_model for prefix in valid_prefixes)
        
        if not is_valid_format:
            # Could be a custom model, so we'll just warn
            print(f"Warning: Embedding model '{embedding_model}' doesn't match common patterns")
    ...
    def test_chunk_settings_logic(self):
        ...
        assert config.CHUNK_SIZE <= 2000, "Chunk size seems too large (> 2000 characters)"
        assert config.CHUNK_OVERLAP <= config.CHUNK_SIZE * 0.5, (
    ...
    def test_api_key_presence(self):
        ...
        if not api_key.startswith('sk-') and not api_key.startswith('test-'):
            print(f"Warning: API key doesn't start with expected prefix 'sk-' (got: '{api_key[:10]}...')")
    ...
    def test_environment_variable_loading(self):
        ...
        assert len(config.ANTHROPIC_API_KEY) > 0, "ANTHROPIC_API_KEY should be loaded from environment"
        assert isinstance(config.ANTHROPIC_API_KEY, str), "ANTHROPIC_API_KEY should be a string"
        
        # Should match what's actually in the environment
        env_api_key = os.environ.get("ANTHROPIC_API_KEY", "")
        assert config.ANTHROPIC_API_KEY == env_api_key, "Config should load ANTHROPIC_API_KEY from environment"
        ...
        assert isinstance(config.ANTHROPIC_MODEL, str), "ANTHROPIC_MODEL should be a string"
        ...
        assert config.ANTHROPIC_MODEL == env_model, "Config should load ANTHROPIC_MODEL from environment or use default"
    ...
    def test_config_values_for_production_readiness(self):
        ...
        if 'MiniLM' in config.EMBEDDING_MODEL:
            ...
            assert 200 <= config.CHUNK_SIZE <= 1200, (
